...
- **Shared expenses**: `Finance::Group` (one per couple, `MAX_MEMBERS = 2`, invite code), `Finance::ExpenseShare` (per-member share, `amount`/`amount_ars`), `Finance::Settlement` (transfer between members). `Finance::Expense#group_id` nil = personal. "My spending" (`/finance/expenses`, charts, personal tool scope) = expenses the user PAID at full amount (`Finance::Expense.paid_by(user)`), adjusted by settlements (sent adds, received subtracts) via `Finance::SpendingQuery`; the split detail (bar, shares, adjust my share) lives in `/finance/shared`. `visible_to(user)` is for authorization only. Balances via `Finance::GroupBalance`. Splits via `Finance::SplitCalculator`.
